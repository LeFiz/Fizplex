...
template <int m> bool Base<m>::invert() {
  etms.clear();
  etms.reserve(m);
  for (auto i = 0; i < m; i++)
...
template <int m> void Base<m>::updateVecWithETM(ETM &etm, SVector &vec) {
...
template <int m> void Base<m>::updateVec(SVector &vec) {
  for (auto &etm : etms)
...
template <int m> void Base<m>::updateUnfinishedEtas(size_t finishedETM) {
